        ...
        render: (_, record) => {
          ...
          return (
            <>
              {isImpactSendTime && (
                <Tooltip
                  title={`Time to send impact for ${currentPeriodName(
                    record,
                    periodNumber,
                  )}`}>
                  <ClockCircleFilled
                    style={{ margin: "0 1px", color: "#F5222D" }}
                  />
                </Tooltip>
              )}
              {isPayMaturityTime && (
                <Tooltip title={"Time to pay maturity"}>
                  <ClockCircleFilled
                    style={{ margin: "0 1px", color: "#FFE800" }}
                  />
                </Tooltip>
              )}
              {isPayInterestTime && (
                <Tooltip
                  title={`Time to pay interest for ${currentPeriodName(
                    record,
                    periodNumber - 1,
                  )}`}>
                  <ClockCircleFilled
                    style={{ margin: "0 1px", color: "#FFC800" }}
                  />
                </Tooltip>
              )}
            </>
      ...
      render: (_, bond) => {
        const percentage = Math.floor(
          ((bond?.issued_amount || 0) /
            (bond?.inner?.bond_units_maxcap_amount || 1)) *
            100,
        );
