...
const InvestorBondActions = ({ bond, mode }) => {
  ...
  const { bondUnitPackageBuy, bondWithdrawEverusd } = usePolkadot();

  const { checkDocumentsFormConfig } = useDocuments({
    state,
    updateState,
    bond,
  });
  ...
  return (
    <>
      <ModalView
        ...
        onCancel={e =>
          stopPropagation(e, () => updateState({ visibleInvestModal: false }))
        }
        width={400}
            ...
            <Menu>
              {["ACTIVE", "BOOKING"].includes(bond.state) && (
                <Menu.Item
                  ...
                  onClick={e =>
                    stopPropagation(e, () =>
                      updateState({ visibleInvestModal: true }),
                    )
                  }>
                  Buy Bond Units
                </Menu.Item>
              ...
              {["ACTIVE", "FINISHED"].includes(bond.state) && (
                <Menu.Item
                  key="withdraw"
                  onClick={e =>
                    stopPropagation(e, () => bondWithdrawEverusd(bond.id))
                  }>
                  Withdraw
                </Menu.Item>
              )}
